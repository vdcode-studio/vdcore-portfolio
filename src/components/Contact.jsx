      ...
      {isOpen && (
        <div onClick={() => setIsOpen(false)} className={`fixed inset-0 z-[99999] flex items-center justify-center p-4 overflow-y-auto ${darkMode ? 'bg-black/60 backdrop-blur-md' : 'bg-slate-900/40 backdrop-blur-md'}`}>
          <div onClick={(e) => e.stopPropagation()} className={`w-full max-w-lg md:max-w-lg p-6 sm:p-10 md:p-8 rounded-[2.5rem] border relative my-auto overflow-hidden ${darkMode ? 'border-indigo-500/35 bg-[#12131c] text-[#f3f4f6] shadow-[0_20px_50px_rgba(0,0,0,0.95)]' : 'border-indigo-200 bg-white text-slate-900 shadow-[0_20px_40px_rgba(99,102,241,0.15)]'}`}>
            ...
            <button onClick={() => setIsOpen(false)} title="Close" aria-label="Close" className={`group absolute top-4 right-4 sm:top-6 sm:right-6 md:top-6 md:right-6 p-2 rounded-full border transition-all duration-300 z-20 cursor-pointer hover:rotate-90 hover:scale-110 ${darkMode ? 'border-indigo-500/30 bg-[#08090e]/80 text-slate-400 hover:text-white hover:border-indigo-400' : 'border-indigo-200 bg-slate-100/80 text-slate-500 hover:text-slate-900 hover:border-indigo-400'}`}>
            ...
            {status.success ? (
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-4 md:gap-4 relative z-10">
                ...
                <div className="flex justify-center mt-1">
                  {/* Send Button */}
                  <button type="submit" disabled={status.loading} title="Send Message" aria-label="Send Message" className={sendButtonClass}>
                    {status.loading ? (
                      <span className="text-xs font-mono font-bold animate-pulse px-1">...</span>
                    ) : (
                      <Send className={`transition-all duration-300 shrink-0 group-hover:scale-110 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 ${
                        darkMode 
                          ? 'w-5 h-5 text-white group-hover:text-indigo-400' 
                          : 'w-5 h-5 text-slate-900 group-hover:text-indigo-600'
                      }`} />
                    )}
                  </button>
                </div>
